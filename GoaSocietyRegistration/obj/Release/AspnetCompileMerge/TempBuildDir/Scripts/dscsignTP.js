...
function CreatePayload(apprefno) {
    ...
    sessionStorage.setItem('card_data', '1');
    if (stack.length == 0) {
        continuePin = "true";
    }
    $.ajax({
        type: 'POST',
        url: "/Organization/Digital/Public?mode=CreatePayloadThirdParty",
...
function DSCSign(payload) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (json.ErrorCode == "") {
                /// $(".sign-status").text("DSC Registered Successfully");
                SavePayload(json.OutputData);
                $(".sign-status").addClass("callout").addClass("callout-danger").addClass("callout-success");
            }
            else {
...
function SavePayload(output) {
    ...
    $.ajax({
        type: 'POST',
        url: "/Organization/Digital/Public?mode=SavePayloadThirdParty",
        data: { Payload: output },
        async: false,
        success: function (json) {
            taskRunning = false;
            ...
            if (json.ErrorCode == "0" || json.ErrorCode == "" || json.ErrorCode == "null") {
                if (stack.length == 0) {
                    ...
                    //window.location.href = "../Organization/Digital/Modal.aspx";
