...
jQuery(this).css('table-layout','fixed');
    ...
    jQuery('.vc_tta-tab a').click(function(){
        ...
        jQuery(href).attr('tabindex',-1);
		jQuery(href).focus();
		if(jQuery(href).find('.vc_tta-panel-body a').length){
        ...
        jQuery(href).removeAttr('tabindex');
		});
		}else{
		dis.closest('li').next().find('a').focus();
	}
